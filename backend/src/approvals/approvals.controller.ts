...
async function fetchApprovals(req: Request, res: Response) {
  try {
    ...
    const approvals = await approvalService.fetchApprovals(userId, role, status, page);
    res.json(approvals);
  } catch (error: any) {
    console.error(error);
    res.status(500).json({ error: error.message });
...
export default { requestKYC, requestRole, approveKYC, approveRole, rejectKYC, rejectRole,fetchApprovals };